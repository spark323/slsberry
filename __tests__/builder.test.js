...
describe('builder', () => {
  ...
  describe('generateOasPaths', () => {
        ...
        api: { // 실제로는 배열이다. (객체처럼 사용되는 배열)
          ...
          post: {
            ...
            parameters: {
              ...
              permission_type: { req: true, type: 'String', desc: 'permission_type' }
            },
            errors: {},
            responses: {
              description: 'ok',
            ...
            requestBody: {
              ...
              content: {
                'application/json': {
                  schema: {
                    type: 'object',
                    required: [ 'key', 'client_id', 'user_id', 'permission_type' ],
                    properties: {
                      key: {
                        description: 'key',
                        type: 'string',
    ...
    test('query parameter 추가가 잘 되나요?', () => {
      const apiSpecList = {
        'api/task': { // 실제로는 배열이다. (객체처럼 사용되는 배열)
          get: {
            category: 'Task',
            event: [ { type: 'REST', method: 'Get' } ],
            ...
            parameters: {
              seq: { req: true, type: 'String', desc: 'seq value to query', in: 'query' }
            },
            responses: {},
            name: 'api/task/get',
            uri: 'api/task',
    ...
    test('path parameter 추가가 잘 되나요?', () => {
      // Mock input parameters
      const apiSpecList = {
        'api/task/{taskId}': { // 실제로는 배열이다. (객체처럼 사용되는 배열)
          get: {
            category: 'Task',
            event: [ { type: 'REST', method: 'Get' } ],
            summary: 'get a task by id',
            ...
            operationId: 'getTaskById',
            parameters: {
              taskId: { req: true, type: 'String', desc: 'task id', in: 'path' }
            },
            responses: {},
      ...
      const apiSpecList = {
        'api/task': { // 실제로는 배열이다. (객체처럼 사용되는 배열)
          get: {
            ...
            operationId: 'getTaskById',
            parameters: {
              customKey: { req: true, type: 'String', desc: 'custom key', in: 'header' }
            },
            responses: {},
            name: 'api/task/get',
            uri: 'api/task',
        ...
        'api/task': { // 실제로는 배열이다. (객체처럼 사용되는 배열)
          post: {
            ...
            parameters: {
              ...
              client_id: { req: true, type: 'String', desc: 'client_id', in: 'body' },
              user_id: { req: true, type: 'String', desc: 'user_id', in: 'body' },
              permission_type: { req: true, type: 'String', desc: 'permission_type', in: 'body' }
            },
            responses: {},
            name: 'api/task/post',
            uri: 'api/task',
